...
async function startCamera() {
  try {
    ...
    message.textContent = "Camera active. Detection runs automatically every 3 seconds.";
...
async function sendFrame() {
  ...
  confidenceText.textContent = `Confidence: ${Number(result.confidence).toFixed(1)}%`;
}
...
async function refreshStatus() {
  ...
  scoreText.textContent = `${Number(data.attention_score).toFixed(1)}%`;
